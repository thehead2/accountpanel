...
if (isset($_POST["submit"]))
	{
		...
		try {
				...
				$donatecoinreward5=1000;
									...
									$balance_final = $donatecoinreward5+$balance;
									
									
									$sql_inv_update = $connection->prepare('REPLACE INTO special_shop_balance (characterId,balance) VALUES(:account,:count)');
						...
						/*$charid_row = $connection->prepare('SELECT address FROM acp_players WHERE account = :charname LIMIT 1');
		$charid_row->execute(array(':charname' => $charname));
		$charid_row_fetch = $charid_row->fetchAll();
		$wallet = $charid_row_fetch[0]['address'];
		

		if(!$wallet|| $wallet=="")
		{
			exit("You need a token wallet first.");
	...
	if (!empty($_POST['enchantsubmit']))
	{
		...
		if ($get_enchant_option != "")
		{
			...
			?>
				...
				<input type="hidden" name="notify_url" value="https://nftlineage2.ddns.net/paypal/system/ipn/ipn_donations.php" />

				<!-- The return page to which the user is navigated after the donations is complete -->
				<input type="hidden" name="return" value="https://nftlineage2.ddns.net/AccountControlPanel/?page=credits.php" />
																			...
																			<?php
																				...
																				if ($donation_select == $donation_option1)
																				{
																					if ($coins7_enabled == true)
																						{
																							?>
																							<option value="<?php echo $donatecoinamount7?>"><?php echo $donatecoinreward7, ' X ', $item_coins_name, ' ', $currency_code_html, $donatecoinamount7;?>.00</option>
																							<?php
																						}
																					if ($coins1_enabled == true)
																						{
																					...
																					if ($coins6_enabled == true)
																						{
																							?>
																							<option value="<?php echo $donatecoinamount6?>"><?php echo $donatecoinreward6, ' X ', $item_coins_name, ' ', $currency_code_html, $donatecoinamount6;?>.00</option>
																							<?php
																						}	
																					if ($coins8_enabled == true)
																						{
																							?>
																							<option value="<?php echo $donatecoinamount8?>"><?php echo $donatecoinreward8, ' X ', $item_coins_name, ' ', $currency_code_html, $donatecoinamount8;?>.00</option>
																							<?php
																						}
																					if ($coins2_enabled == true)
																						{
																					...
																					if ($coins5_enabled == true)
																						{
																							?>
																							<option value="<?php echo $donatecoinamount5?>"><?php echo $donatecoinreward5, ' X ', $item_coins_name, ' ', $currency_code_html, $donatecoinamount5;?>.00</option>
																							<?php
																						} 																						
																				}
																				// KARMA
